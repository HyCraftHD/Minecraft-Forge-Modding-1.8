...
public class BlockDimPortal extends BlockBreakable {
	...
	public boolean func_176548_d(World worldIn, BlockPos p_176548_2_) {
		BlockDimPortal.Size size = new BlockDimPortal.Size(worldIn, p_176548_2_, EnumFacing.Axis.X);

		if (size.func_150860_b() && size.field_150864_e == 0) {
			size.func_150859_c();
			return true;
		} else {
			BlockDimPortal.Size size1 = new BlockDimPortal.Size(worldIn, p_176548_2_, EnumFacing.Axis.Z);
	...
	public void onNeighborBlockChange(World worldIn, BlockPos pos, IBlockState state, Block neighborBlock) {
		...
		if (axis == EnumFacing.Axis.X) {
			size = new BlockDimPortal.Size(worldIn, pos, EnumFacing.Axis.X);

			if (!size.func_150860_b() || size.field_150864_e < size.field_150868_h * size.field_150862_g) {
				worldIn.setBlockState(pos, Blocks.air.getDefaultState());
			}
		} else if (axis == EnumFacing.Axis.Z) {
			size = new BlockDimPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
